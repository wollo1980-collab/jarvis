...
## 2026-07-10 - **v1.0 getaggt** — Nutzungslauf abgenommen (PO-Freigabe)

**PO-Entscheidung „Ja taggen":** Die 1.0-Definition (Beschluss 09.07.: Welle 1 ✓ + Welle 2 ✓ + dokumentierter realer Nutzungslauf ohne kritischen Befund, KEINE 7-Tage-Wartezeit) ist erfüllt. Der Nutzungslauf 09./10.07. lieferte ~20 Befunde (stumme OpenAI-TTS/WAV-Header, Roboterstimme, vorgelesene URLs, Wake-Word-Endlos-Schleife, verpasste leise Sprecher, News-Ballast, …) — **alle behoben, alle vom PO live nachgetestet**, keiner kritisch (kein Datenverlust, kein Sicherheitsloch, kein Totalausfall ohne Ausweg). Genau diese Schleife — bauen → real benutzen → Reibung → Fix → erneut benutzen — war das Framework-Pattern „Nutzungslauf vor Abschluss"; es hat sich hiermit zum zweiten Mal bewährt (M4/n=2-Rückfluss-Kandidat fürs Framework). Tag `v1.0` auf den grünen Stand nach Welle 3.4 (561 Tests). Wellen 3.3 (Telegram-Bestätigungen) und 4 (Schaufenster) laufen als 1.x weiter.

## 2026-07-10 - Welle 3.4 `restart_runtime`: „Starte dich neu"

**Nutzungslauf-Befund 09.07. geschlossen:** Beenden per Zuruf war komfortabel, Neustarten (Doppelklick) umständlich — und im laufenden Nutzungslauf ist Neustarten nach jedem Fix der häufigste Handgriff überhaupt. **Umsetzung:** Command `restart_runtime` (Stufe 0, gleiches Entkopplungs-Muster wie `stop_runtime`: injizierter Hook, Fachlogik kennt die Runtime nicht) → `_request_restart` startet ERST den abgekoppelten Nachfolger-Prozess (gleicher Interpreter/Entry-Point, DETACHED_PROCESS), DANN Stop-Sentinel — scheitert der Spawn, fährt nichts herunter (Hook meldet False, Antwort ehrlich: „ich bleibe im Dienst"). **Lock-Staffelstab (der Knackpunkt):** Der Nachfolger erbt `JARVIS_WAIT_FOR_LOCK=30` und `SingleInstanceLock.acquire(retry_seconds=…)` pollt bis zur Freigabe des Vorgängers, statt sofort zu sterben; ohne das Env-Flag (normaler Start/Doppelstart) bleibt ADR-026 unverändert hart — das Flag wird nach dem Erwerb aus dem Env entfernt (keine stille Vererbung). Planner-Guidance + Notfall-Heuristik („starte dich neu" wirkt auch bei toter API, geprüft VOR stop_runtime — nie als bloßes Beenden fehldeuten), Telegram-Runtime-Whitelist (Standalone-Bot bewusst nicht — kein Hook). Kein neues ADR: Komposition bestehender Entscheidungen (ADR-026-Lock, Runtime-Lebenszyklus, Zusage-vor-Teardown).

## 2026-07-10 - News-Briefing entschlackt: drei Befunde, eine Wurzel

**Nutzungslauf-Befund:** (1) „(tagesschau.de - die erste Adresse für Nachrichten und Information)" klebte an jeder Meldung, (2) eine Meldung kam als Titel-Dublette („Merz verteidigt … — Merz verteidigt …"), (3) Meldung 4 wurde gar nicht vorgelesen. Wurzel von (3) waren (1)+(2): Der redundante Ballast fraß den 600-Zeichen-Sprech-Deckel auf, bevor alle Meldungen dran waren — die Meldung stand im Text, fiel aber beim Sprechen hinten runter. **Fixes:** Feed-Titel-Slogans abschneiden (`source.split(" - ")[0]`); Einzelquelle einmal in den Kopf statt an jede Zeile (bei Quellen-Mischung bleibt die Zuordnung pro Meldung — PIS-Belege-Prinzip unangetastet); `_dedupe_summary` verwirft descriptions, die nur den Titel spiegeln; der Füllstoff-Filter (bisher nur Themen-Suche) greift jetzt überall und kennt zusätzlich „Livestream"-Programm-Promo. Lehre: Ein Sprech-Längen-Deckel macht Redundanz zum Verdrängungsproblem — was doppelt gesagt wird, verdrängt, was gesagt werden sollte.

## 2026-07-09 - Bugfix: Wake-Aufnahme verpasste leise Sprecher (Schwelle jetzt selbstkalibrierend)

**Nutzungslauf-Befund (Log-Beweis):** „Ja, Sir?" kam, die Frage wurde gestellt — dann Ende-Piep, keine Antwort. Der Log zeigte zweimal „Wake-Word ohne Anschlussfrage - verworfen": Die Frage lag unter der festen Sprach-Schwelle (RMS 300). Auffällig auch: Wake-Scores 0.72/0.60 statt 0.96–1.00 — das Signal war insgesamt leiser (Abstand/Pegel). **Fix:** Die Schwelle wird nicht mehr geraten, sondern **am gerade gehörten „Hey Jarvis" kalibriert** — der Lauscher merkt sich die Lautstärke der letzten ~1 s (das Wake-Word selbst), Schwelle = 25 % dieses Stimmpegels, gedeckelt bei 300 (alte Konstante als Obergrenze), Untergrenze 60 gegen Rauschen. Wer leise ruft, dessen leise Frage wird jetzt auch gehört. **Observability:** Trigger-Log nennt jetzt Stimmpegel und Schwelle; ein „verworfen" loggt den Spitzenpegel des Vorlaufs — das nächste Live-Problem ist damit Ablesen statt Rätselraten (nur Zahlen, nie Inhalte).

## 2026-07-09 - Wake-Bestätigung „Ja, Sir?" statt Piepton

**Nutzungslauf-Wunsch:** „vielleicht sollte auch kein Piepen kommen sondern ein Ja Sir oder so?" — der Piepton ist Alexa, „Ja, Sir?" ist Jarvis. **Kernproblem:** Live-TTS braucht 1–3 s und würde den 4-s-Sprach-Vorlauf auffressen. **Lösung:** Die Bestätigung wird beim Start **einmal** synthetisiert (gleiche Stimme wie die Antworten) und als WAV-Bytes im Speicher gecacht; `winsound.PlaySound(…, SND_MEMORY)` spielt sie dann quasi verzögerungsfrei ab — so schnell wie der Beep. Während der Wiedergabe ist `_speaking` gesetzt (die eigene Bestätigung weckt nicht erneut), die Aufnahme startet erst danach (blockierende Wiedergabe). `wake_acknowledgement` konfigurierbar (leer = Piepton); scheitert die Start-Synthese, bleibt der Piepton automatischer Rückfall — der Zuruf funktioniert also auch offline weiter. Ende-Piep (Aufnahme geschlossen) bewusst behalten: kurzes, eindeutiges „gehört"-Signal.

## 2026-07-09 - Welle 3.2b Wake-Word „Hey Jarvis" (ADR-044): der Zuruf

**Machbarkeits-Fund:** openwakeword läuft auf Py 3.13 und bringt ein vortrainiertes **`hey_jarvis`**-Modell mit (1,3 MB ONNX; Stille-Inferenz sauber 0.0). **Umsetzung:** `WakeWordListener` im Sprachkanal — Dauerstream 16 kHz → 80-ms-Frames → lokales Modell → Schwelle 0.5 → Signalton → **Aufnahme bis ~1,5 s Stille** (RMS, in *Audio-Zeit* gerechnet — deterministisch testbar) → geteilte `process_audio`-Pipeline (aus dem Hotkey-Pfad extrahiert; Hotkey bleibt parallel). **Selbstschutz:** `_speaking`-Flag um die TTS-Wiedergabe — die eigene Stimme weckt Jarvis nicht; 3 s Abklingzeit gegen Doppel-Trigger. **Privacy (ADR-Kern):** jedes Frame nur lokal bewertet und verworfen; nichts persistiert/gesendet vor dem Wake-Word; `wake_word_enabled` default **aus** (Privacy-by-default/ADR-037), beim PO an. Deps: openwakeword+onnxruntime (requirements-runtime). Ehrlich: Fehl-Auslöser/Verpasser sind bei Wake-Words normal — Schwelle wird im Nutzungslauf justiert; Hotkey = deterministischer Rückfall. **Damit sind alle drei Sprach-Zugänge komplett: Telegram-Voice (ADR-038), Push-to-talk (ADR-041), Zuruf (ADR-044).**

## 2026-07-09 - Doppel-Scheibe: Orts-News + Wetter (ADR-043)

**Nutzungslauf-Wünsche direkt nach dem News-Briefing:** (1) `get_news` um `topic`-Parameter erweitert — Google-News-**RSS-Suche** liefert regionale/thematische Schlagzeilen im selben Format wie die Standard-Feeds (gleicher Reader, nur URL-Weiche; kein Key). (2) Neue Fähigkeit `get_weather` + Connector `core/weather.py` über **Open-Meteo** (keyless): Geocoding erster Treffer, 7-Tage-Forecast, WMO-Code→Deutsch-Mapping, `parameters.day` = heute/morgen/übermorgen/ISO (Planner rechnet Wochentage dank Datum-Injektion um, Deckel 6 Tage). Standard-Ort `weather_default_location` (PO: Usingen); genannter Ort gewinnt pro Frage. Beide Stufe 0, Standalone-Whitelist, Persona-Ton. **Nebenbei:** Der Phantom-Intent-Wächter von 2026-07-01 (`"weather" not in prompt`) hat ausgedient — get_weather existiert jetzt wirklich; Test auf Positiv-Prüfung umgestellt. Bewusst nicht: Stundenvorhersage, Unwetterwarnung, Geocoding-Mehrdeutigkeits-Rückfrage (ADR-043 „Bewusst NICHT").
...
## 2026-07-09 - A2 „Scheduler + Push" (ADR-039): die erste Proaktivität — Welle 1 komplett

**Architektur-Novum, sauber eingezäunt:** Jarvis initiiert erstmals Nachrichten — ADR-039 ordnet das als **Auftragserfüllung** ein (expliziter „erinnere mich…"-Auftrag, rein informativ, Stufe 0; Governance-Invariante gewahrt; jede weitergehende Proaktivität braucht ein eigenes ADR). Umsetzung: runtime-eigener **Scheduler-Thread** (Polling 30 s, `stop_event.wait` als Sleep+Stop-Signal), persistiertes **`notified`-Flag** (einmaliges Feuern; `add()` markiert Vergangenes sofort → rückdatierte Merkposten feuern nie; Migration für A1-Einträge beim Lesen), **at-most-once** (markieren vor Send — kein Wiederhol-Spam, Verlustfall geloggt), **Nachholung** mit ehrlichem „verspätet"-Label (≥2 min; ganztägige nie verspätet, kommen morgens). Push entkoppelt: `TelegramChannel.push()` (gleiche Loop-Brücke + Pending-Tracking wie Replies), `main()` injiziert ihn als Notifier — ohne Kanal kein Scheduler. Runtime + Commands teilen **eine** Store-Instanz (`configure()` gibt sie zurück). Damit ist **Welle 1 des Offensiv-Fahrplans komplett**: ansagen → verstehen → erinnern → **von selbst melden**.

## 2026-07-09 - Welle 1.2+1.3: „Meister"-Fix (Vergessen wirkt wirklich) + sichtbares Gedächtnis

**1.2 Bugfix (Dogfooding-Fund 08.07.):** Ein per `forget_fact` gelöschter Fakt („nenn mich Meister") lebte im Gesprächsverlauf weiter — das Modell führte das Muster der letzten 20 Nachrichten fort. Fix: `build_chat_system_prompt()` hängt eine **Vorrang-Regel** an (aktueller Langzeitgedächtnis-Stand schlägt ältere Verlaufs-Aussagen, v. a. Anrede/Präferenzen) — **auch bei leerem Gedächtnis** wird der Stand jetzt explizit genannt (sonst greift die Regel genau dann nicht, wenn der gelöschte Fakt der einzige war). Zusätzlich entwertet die `forget_fact`-Bestätigung den Fakt ausdrücklich („…gilt ab sofort nicht mehr"). Ehrlich: Prompt-Steuerung, kein harter Garant — Live-Beweis per Meister-Szenario. **1.3 Sichtbares Gedächtnis (PIS-Prinzip):** neuer read-only Befehl `list_facts` („was hast du dir gemerkt?") zeigt alle Fakten mit Kategorie; in der Standalone-Whitelist (Stufe 0, wie remember/forget). Planner-Guidance inkl. Abgrenzung zu `list_entries` und „merk dir X nicht mehr" → `forget_fact`.

## 2026-07-09 - A1 „Einträge": Erinnerungen/Aufgaben/wichtige Merkposten (Welle 1 Offensiv-Fahrplan)

**Erste Scheibe des freigegebenen Offensiv-Fahrplans.** Vereinheitlichtes Datenmodell (ein Store für Erinnerung + Aufgabe/Notiz + wichtigen Merkposten): `Eintrag = {id, text, when (ISO, optional), important, created}` in neuem `memory/entries.py` (atomar via fileio, RLock — vorwärtskompatibel zum A2-Scheduler). Drei Commands (`commands/entries.py`): `add_entry`/`list_entries`/`delete_entry`, Echo mit lesbarer Zeit + ⭐ bei wichtig (Verhörer-/Datumsfehler-Kontrolle). **Planner kennt jetzt „jetzt":** `build_system_prompt()` injiziert aktuelles Datum/Uhrzeit (Europe/Berlin, dt. Wochentag) — nötig, damit „morgen um 9" → ISO umgerechnet wird; Guidance grenzt Einträge streng von `remember_fact` (dauerhafte Fakten) ab. Listen-Default (PO): offene/zukünftige + **alle** wichtigen; nicht-wichtige Vergangenheit nur per Filter; reines Datum zählt bis Tagesende als offen; unparsebares `when` bleibt fail-safe sichtbar. Whitelist: die 3 Intents nur im Runtime-Telegram-Kanal. **Noch ohne Feuern** — proaktiver Push ist A2 (ADR-039).

## 2026-07-08 - Sprach-Eingabe (STT) über Telegram (ADR-038): Aufgaben/Erinnerungen ansagen

**Alltagstauglichkeit, Zugang-Hebel:** Jarvis nimmt jetzt über den Runtime-Telegram-Kanal **Sprachnachrichten** entgegen (`_on_voice`) — der Türöffner für „Aufgaben & Erinnerungen ansagen" (nächster Wert-Schritt). Neues `core/transcribe.py` (`OpenAITranscriber`, Geschwister des OpenAI-TTS-Backends, derselbe `openai_api_key`, kein neues pip-Paket). PO-Auflagen umgesetzt: **Autorisierung vor jedem Download**, **Audio nur im Speicher** (`download_as_bytearray`, kein Datei-Write), **Transkript-Echo** (🎤 „Verstanden: …"), **Fehler/leeres Transkript → keine Ausführung**. Das Transkript läuft durch **dieselbe Whitelist/Pipeline** wie Text (kein neuer Sicherheitspfad); der fail-closed `listen()`-Adapter bleibt unberührt. Ohne OpenAI-Key wird der Voice-Handler nicht registriert (Textbetrieb unverändert). Datenschutz (Stimme → OpenAI) bewusst in ADR-038 dokumentiert. Nebenbei: Reply-Callback in `_make_reply_callback` extrahiert (Text- + Voice-Pfad teilen ihn).
