...
## Current Version
`v0.7.0` (erster Baustein: PC-Analyse, Phase 1) - `v0.7` als Gesamtversion ist NOCH NICHT getaggt (weitere Kap.-17-Bausteine offen). `v0.4`/`v0.5`/`v0.6` bleiben abgeschlossen und getaggt. Handbook ist auf `v3.5` (inkl. Kap.-2-Konsistenzkorrektur, siehe Git-Historie).
...
- Tests: `164 / 164` grün.
- Windows-Ereignisprotokoll, Optimierung/Bereinigung, Registry-Änderungen, Dienste, Treiber sind bewusst NICHT enthalten (Phase 1).
...
`v0.7 "PC-Admin"` laut Handbook Kap. 13 - Phase 1 (System-Analyse: Festplatte/Prozesse/Autostart) umgesetzt. Weitere Kap.-17-Bausteine (Treiber, Dienste, Bereinigung, Ereignisprotokoll) NICHT begonnen - naechste Priorisierung liegt beim Product Owner.

## Next Planned Version
Weiterhin `v0.7` - noch NICHT abgeschlossen/getaggt. Offene Entscheidung: naechsten Kap.-17-Baustein priorisieren (z. B. Ereignisprotokoll, Autostart-Verwaltung mit Schreibzugriff, Bereinigung, Dienste, Treiber) oder v0.7 mit dem aktuellen Umfang als abgeschlossen betrachten und taggen. Kein technischer Vorschlag fuer einen weiteren Baustein bisher erstellt, kein Code geschrieben.

## Next Goal According To Handbook
`v0.6` ist inhaltlich vollstaendig und im Handbook (v3.5) selbst als abgeschlossen markiert. `v0.7 "PC-Admin"` (Kap. 13/17) hat mit Phase 1 (System-Analyse: Festplatte/Prozesse/Autostart, nur lesen) begonnen (ADR-020).
Kap. 17 nennt fuer v0.7 weiterhin offen: Treiber pruefen/aktualisieren, Dienste starten/stoppen, Autostart *verwalten* (Phase 1 deckt nur *anzeigen* ab), Temp-/Festplatten-Bereinigung, Windows-Ereignisprotokoll analysieren.
Vor jedem weiteren Baustein gilt weiterhin: Handbook-Pruefung (Scope/DoD/Architektur/Sicherheitsmodell) + technischer Vorschlag zur Freigabe durch den Product Owner, bevor Code geschrieben wird (Muster aus ADR-013 bis ADR-020).
...
- `v0.7`, weitere Kap.-17-Bausteine: Windows-Ereignisprotokoll analysieren, Treiber pruefen/aktualisieren, Dienste starten/stoppen, Autostart *verwalten* (Schreibzugriff, ueber Phase-1-Anzeige hinaus), Temp-/Festplatten-Bereinigung - keine Priorisierung, kein Code.
- Dritter KI-Verwender: falls ein weiteres Modul KI-Zugriff braucht, `configure()`-Duplizierung (`reports.py`/`monitor.py`) zu einer gemeinsamen Abstraktion zusammenfuehren pruefen (Wolfgangs Entscheidung bei ADR-020).
...
`commands/monitor.py::AnalyzePcCommand` (Intent `analyze_pc`, Sicherheitsstufe 0) ist der zweite Command mit direktem `AIEngine`-Zugriff (nach `analyze_report`/`calculate_kpi`) - eigenes, zu `reports.py` bewusst dupliziertes `configure()`-Muster statt einer gemeinsamen Abstraktion (Wolfgangs Entscheidung, wird bei einem dritten Verwender neu geprueft). Gleicher `TYPE_CHECKING`-Zirkelimport-Schutz wie ADR-015, diesmal proaktiv angewendet. `winreg`-Import ueber `try/except ImportError` abgesichert (Windows-exklusiv). Keine Aenderung an `core/ai.py`, `core/planner.py`, `core/tool_manager.py`, `executor/executor.py` oder anderen `commands/*.py`-Dateien (per `git diff --stat` verifiziert leer).
...
- `analyze_pc`: Windows-exklusiv (klarer Fehler auf anderen Plattformen), keine Aufloesung von Startup-Ordner-Verknuepfungszielen (nur Dateinamen), kein Ereignisprotokoll (ADR-020).

## Git
Ein einzelner, ehrlicher Initial-Commit aus dem aktuellen Arbeitsstand (kein rekonstruierter Verlauf aus alten ZIP-Staenden), getaggt als `v0.4`. Danach je ein Commit fuer Handbook v3.3/ADR-013, Excel-Lesen (ADR-014), Tabellen-Auswertung (ADR-015), die Power-BI-Scope-Entscheidung, KPI (ADR-016) und die v0.5-Abschlusspruefung, getaggt als `v0.5`. Danach Commits fuer Handbook v3.4/ADR-017, Telegram-Fernzugriff (ADR-018), getaggt als `v0.6`, danach Handbook v3.5/ADR-019 inkl. einer kleinen Kap.-2-Konsistenzkorrektur. Fruehere Versionen (v0.1-v0.3) existieren nur als Text in `docs/CHANGELOG.md`/`docs/logbook.md`, nicht als eigene Git-Commits/Tags (keine kuenstliche Fake-Historie). Der PC-Analyse-Baustein (dieser Stand) ist noch nicht committed. Kein neuer Tag - `v0.7` wird erst bei Abschluss aller vorgesehenen v0.7-Bausteine getaggt.
