...
# _read_text_capped lebt jetzt geteilt in core/fileio.py (read_text_capped) -
# project_continue (commands/delegate.py) braucht denselben gedeckelten Leser.
_read_text_capped = read_text_capped
...
_STATUS_OPEN_RE = re.compile(r"<!--\s*status:\s*offen\s*-->", re.IGNORECASE)


def _supersede_open_proposals() -> None:
    """Automatik (Audit-Fund 2, PO-Entscheidung 11.07.2026): sobald ein NEUER
    naechster Schritt vorgeschlagen wird, ist der vorherige nicht mehr 'der
    naechste Schritt' - er wurde umgesetzt oder ist ueberholt. Deshalb alle
    bisher OFFENEN Vorschlaege beim Anlegen des neuen automatisch auf
    'abgeloest' setzen. Ohne das blieb ein umgesetzter Vorschlag fuer immer
    als 'offen'-Karte sichtbar (kein Code-Pfad aenderte je den Status).
    Fail-safe: ein Fehler hier darf das Schreiben des neuen nie verhindern."""
    if _proposals_dir is None:
        return
    try:
        stamp = datetime.now().isoformat(timespec="seconds")
        for path in _proposals_dir.glob("*.md"):
            try:
                content = path.read_text(encoding="utf-8")
            except OSError:
                continue
            if not _STATUS_OPEN_RE.search(content):
                continue
            new_content = _STATUS_OPEN_RE.sub(
                f"<!-- status: abgeloest (neuer Vorschlag {stamp}) -->", content, count=1
            )
            path.write_text(new_content, encoding="utf-8")
            logger.info("Vorschlag automatisch abgeloest: %s", path.name)
    except OSError as e:  # noqa: BLE001 - Datei-Aufraeumung ist Beiwerk
        logger.warning("Alte Vorschlaege konnten nicht abgeloest werden: %s", e)
...
def _write_proposal(text: str) -> Optional[Path]:
    ...
    if _proposals_dir is None:
        return None
    try:
        # Automatik (Audit-Fund 2): vorherige offene Vorschlaege abloesen,
        # BEVOR der neue entsteht - danach ist genau dieser der offene.
        _supersede_open_proposals()
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        # Status-Konzept (PO-Go 11.07.2026, Vorschlags-Karte): offen ->
        # umgesetzt/verworfen/abgeloest. Nur "offen" erscheint als Karte.
        # Neue Vorschlaege starten offen; die Automatik oben haelt hoechstens
        # EINEN offen (den juengsten).
        header = (
...
class DismissProposalCommand:
    """Verwirft Jarvis' offenen Eigenvorschlag per Zuruf - das Chat-Gegenstueck
    zum ✕ an der Vorschlags-Karte (PO-Reibung 2026-07-11: 'Deinen Entwurf
    verwerfen' fand kein Intent und wurde vom Planner faelschlich als
    stop_runtime gedeutet -> Jarvis fuhr herunter). Mit einem eigenen Intent
    hat 'verwerfen' ein richtiges Zuhause."""

    name = "dismiss_proposal"
    description = (
        "Verwirft Jarvis' OFFENEN Eigenvorschlag (die »Jarvis schlägt vor«-Karte). "
        "Trigger z. B. 'verwirf den Vorschlag', 'lehn den Entwurf ab', 'den Vorschlag "
        "brauchen wir nicht', 'weg mit dem Vorschlag', 'deinen Entwurf verwerfen'. "
        "NICHT herunterfahren, NICHT eine Notiz/Erinnerung/einen Fakt löschen - es "
        "geht NUR um Jarvis' eigenen Schritt-Vorschlag. Sicherheitsstufe 0."
    )
    requires_confirmation = False  # eigener Vorschlag, reversibel (Status im Artefakt)
    ...
    def execute(self, plan: Plan) -> Result:
        ...
        if _proposals_dir is None:
            return Result(
                status=Status.FAILED,
                message="Vorschlags-Ablage nicht konfiguriert - ich kann nichts verwerfen.",
            )
        memory_dir = _proposals_dir.parent
        proposal = open_proposal(memory_dir)
        if proposal is None:
            return Result(
                status=Status.SUCCESS,
                message="Es steht gerade kein offener Vorschlag an, Sir - nichts zu verwerfen.",
            )
        if dismiss_proposal(memory_dir, proposal["file"]):
            return Result(
                status=Status.SUCCESS,
                message=f"Erledigt, Sir - der Vorschlag »{proposal['title']}« ist verworfen.",
            )
        return Result(
            status=Status.FAILED,
            message="Der Vorschlag ließ sich nicht verwerfen (Details im Log).",
        )


# Registrierungspunkt - commands/__init__.py liest diese Liste beim Start ein.
COMMANDS = [PlanNextStepCommand(), DismissProposalCommand()]
