...
analyze_pc (ADR-020) ruft direkt die KI auf: Python sammelt und
strukturiert alle Daten deterministisch, die KI formuliert nur den
Bericht und interpretiert Auffaelligkeiten - sie rechnet nichts nach.
Eigenes configure()-Muster (AIEngine injiziert), wie beim
Langzeitgedaechtnis (ADR-009).
...
# Pflicht-Hinweis (PO-Entscheidung, siehe ADR-015): KI-Analyse
# kann falsch liegen - Jarvis behauptet keine geschaeftskritische
# Wahrheit, sondern liefert einen Assistenzhinweis.
...
# Anzahl statt Zeitraum-Filter (PO-Vorgabe erlaubte beides) - konsistent
...
def configure(ai_engine: "AIEngine") -> None:
    ...
    commands/memory.py, ADR-009) - die Registry instanziiert Commands
