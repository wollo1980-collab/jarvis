...
def _briefing(relevant: list[MailHeader], ads: list[MailHeader], errors: list[str], *, show_ads: bool) -> str:
    ...
    if relevant:
        ...
        if show_ads:
            lines.append(f"Nichts Dringendes, Sir. Hier sind die {len(ads)} Werbe-/Newsletter-Mail(s):")
            lines += [_line(h) for h in ads]
        else:
            lines.append(
                f"Nichts Dringendes, Sir - nur {len(ads)} Werbe-/Newsletter-Mail(s), "
                "ausgeblendet. Sag 'zeig die Werbung', wenn du sie dennoch sehen willst."
            )
    else:
        lines.append("Nichts Neues, Sir. Die Postfächer sind ruhig.")
