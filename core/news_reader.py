...
def google_news_feed_url(topic: str) -> str:
    ...
    oder Thema ("Berlin", "Bitcoin") im selben RSS-Format wie die
...
# 2026-07-09; PO-Reibung 2026-07-11 "Livestream passt nicht"): auto-generierter
# Junk (Luftqualitaet/Pollenflug/Spritpreis) und Programm-Promo ("Livestream:
# ...") - fuer Wetter gibt es die eigene Faehigkeit. Kanonisch HIER, damit
# JEDER Konsument (gesprochene News UND das Dashboard-Panel "Die Lage") den
# Junk los ist - frueher filterte nur commands/news.py, das UI nicht.
JUNK_HEADLINE_RE = re.compile(
...
def fetch_headlines(
    ...
    for url in feeds:
        try:
            ...
            # Junk gleich an der Quelle aussieben (Livestream-Promo & Co.) -
            # so bleiben Dashboard-Panel und gesprochenes Briefing sauber.
            headlines = [h for h in headlines if not JUNK_HEADLINE_RE.search(h.title)]
            if headlines:
