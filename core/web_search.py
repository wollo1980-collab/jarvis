...
# Retry NUR fuer fluechtige Verbindungsabbrueche (Nutzungslauf-Befund
# 2026-07-10: WinError 10054 "Verbindung vom Remotehost geschlossen" riss
# eine einzelne Sprach-Recherche ab; der naechste Versuch klappt praktisch
# immer). Offline-/DNS-Fehler und Timeouts werden bewusst NICHT wiederholt -
# dort ist schnelles, ehrliches Scheitern besser als 3x warten.
_RETRY_ATTEMPTS = 3
_RETRY_BACKOFF_SECONDS = 0.6


def _is_transient(error: OSError) -> bool:
    """Verbindungs-Reset/-Abbruch (auch als URLError.reason verpackt) gilt
    als fluechtig und einen weiteren Versuch wert."""
    candidates = [error]
    reason = getattr(error, "reason", None)
    if isinstance(reason, BaseException):
        candidates.append(reason)
    for candidate in candidates:
        if isinstance(candidate, (ConnectionResetError, ConnectionAbortedError, BrokenPipeError)):
            return True
        if getattr(candidate, "winerror", None) in (10053, 10054):
            return True
    return False
    ...
    for attempt in range(1, _RETRY_ATTEMPTS + 1):
        try:
            ...
            if attempt < _RETRY_ATTEMPTS and _is_transient(e):
                logger.warning(
                    "Websuche: fluechtiger Verbindungsabbruch (Versuch %d/%d): %s",
