...
def test_executor_reports_step_progress_via_on_step():
    """Live-Ablauf 2026-07-10: on_step meldet jeden Schritt bei Beginn und
    Abschluss - und ein kaputter Callback stoert die Ausfuehrung nie."""
    speech = MagicMock()
    ai = MagicMock()
    command = MagicMock(spec=["execute"], )
    command.execute.return_value = Result(status=Status.SUCCESS, message="ok")
    tool_manager = MagicMock(resolve=MagicMock(return_value=command))
    executor = Executor(speech, ai, tool_manager=tool_manager)
    calls = []
    ...
    report = executor.run(
        [Plan(intent="a", raw_input="x"), Plan(intent="b", raw_input="y")],
        on_step=lambda phase, index, step, result=None: calls.append((phase, index, step.intent)),
    )

    assert report.all_ok
    assert calls == [("start", 0, "a"), ("done", 0, "a"), ("start", 1, "b"), ("done", 1, "b")]

    # Kaputter Callback: Ausfuehrung laeuft trotzdem durch.
    def boom(*args, **kwargs):
        raise RuntimeError("UI weg")

    report = executor.run([Plan(intent="a", raw_input="x")], on_step=boom)
    assert report.all_ok
