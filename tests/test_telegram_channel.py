...
def _make_channel(allowed_chat_id="111"):
    ...
    # Echtes Gate statt MagicMock: offer_answer() eines Mocks waere truthy
    # und wuerde jede Nachricht faelschlich als Bestaetigungs-Antwort
    # konsumieren (ADR-045).
    channel.gate = ConfirmationGate()
    return channel
...
def test_runtime_whitelist_allows_confirmation_gated_commands():
    """ADR-045: Mit dem ConfirmationGate haben Stufe-2/3-Befehle einen echten
    Bestaetigungsweg - alle 7 sind im Runtime-Kanal freigeschaltet (PO
    10.07.2026), im Standalone-Bot (kein Gate) bewusst weiterhin nicht."""
    gated = {
        "shutdown_pc",
        "clean_temp_files",
        "install_program",
    ...
    steps, rejection = telegram_channel._runtime_filter_plan([Plan(intent="shutdown_pc")])
    assert rejection is None
    assert len(steps) == 1


def test_telegram_confirmer_bridges_reply_and_gate():
    """_TelegramConfirmer: say() geht ueber den Antwortweg des ausloesenden
    Chats, listen() wartet auf die Antwort aus dem ConfirmationGate."""
    import threading
    import time

    from core.confirmation import ConfirmationGate
    from telegram_channel import _TelegramConfirmer

    sent = []
    gate = ConfirmationGate()
    confirmer = _TelegramConfirmer(sent.append, gate)

    confirmer.say("Kritische Aktion. Bitte tippe zur Bestätigung genau: HERUNTERFAHREN")
    assert sent and "HERUNTERFAHREN" in sent[0]

    received = {}
    t = threading.Thread(target=lambda: received.setdefault("a", confirmer.listen()))
    t.start()
    for _ in range(200):
        if gate.offer_answer("HERUNTERFAHREN"):
            break
        time.sleep(0.01)
    t.join(timeout=5.0)
    assert received["a"] == "HERUNTERFAHREN"
...
def test_runtime_allows_list_intents_but_standalone_not():
    """Listen (2026-07-10): die Einkaufsliste soll gerade unterwegs
    erreichbar sein - Runtime-Kanal ja, Standalone-Bot bleibt minimal.
    clear_list ist dank Papierkorb+restore_list gefahrlos."""
