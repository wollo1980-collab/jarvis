...
PO-Entscheidung: einfacher, kein öffentlich erreichbarer Server,
passend für einen privaten Start).

Sicherheitsmodell (Phase 1, PO-Entscheidungen):
...
(PO-Entscheidung) - beide teilen sich dieselben memory_data/-
...
def rejection_reason(
    step: Plan, allowed: Optional[set] = None, allow_confirmation: bool = False
) -> Optional[str]:
    ...
    allow_confirmation (ADR-045): NUR der Runtime-Kanal mit ConfirmationGate
    setzt True - dort haben Stufe-2/3-Befehle einen echten Bestaetigungsweg
    (der Executor-Dialog uebernimmt die Kontrolle). Default False: der
    whitelist-unabhaengige Waechter bleibt fuer den Standalone-Bot
...
def filter_plan(
    steps: list[Plan], allowed: Optional[set] = None, allow_confirmation: bool = False
) -> tuple[list[Plan], Optional[str]]:
    """Prüft alle Schritte eines Plans. Ist auch nur einer nicht erlaubt,
    wird der GESAMTE Plan abgelehnt (PO-Entscheidung) - liefert
    ([], Ablehnungsgrund). Sind alle Schritte erlaubt, liefert (steps, None).

    allowed (optional, ADR-035) und allow_confirmation (ADR-045) werden an
    rejection_reason durchgereicht; Defaults = Standalone-Verhalten
    unveraendert (kein Stufe-2/3 ohne Bestaetigungsweg)."""
    for step in steps:
        reason = rejection_reason(step, allowed, allow_confirmation)
        if reason:
